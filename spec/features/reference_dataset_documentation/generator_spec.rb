...
describe ReferenceDatasetDocumentation::Generator do
  ...
  describe "#perform" do
    ...
    it "writes an intro and uses TreeWriter and FileListWriter for the rest" do
      ...
      expect(generator).to receive(:add).
        with("# Directory structure '#{root_deployment_name}' for '#{example_type}' example", "")
  ...
  describe "#config_repo_path" do
    ...
    it "return the path of the config repo" do
      expect(generator.config_repo_path).to eq("#{base_dir}/docs/reference_dataset/#{config_repository}/#{example_type}")
    end
  end
  ...
  describe "#template_repo_path" do
    ...
    it "return the path of the config repo" do
      expect(generator.template_repo_path).to eq("#{base_dir}/docs/reference_dataset/#{template_repository}/#{example_type}")
    end
  end
