...
describe 'generate-depls for bosh precompile pipeline' do
  let(:ci_path) { Dir.pwd }
  ...
  context 'when a simple deployment is used' do
    ...
    context 'when valid' do
      let(:options) { "-d #{depls_name} -o #{output_path} -t #{templates_path} -p #{secrets_path} --iaas #{iaas_type} --no-dump -i bosh-precompile --profiles #{profiles.join(',')}" }
      ...
      it 'only bosh-precompile template is processed' do
        expect(stdout_str).to include('processing ./concourse/pipelines/template/bosh-precompile-pipeline.yml.erb').and \
          include('1 concourse pipeline templates were processed')
      end

      context 'when pre-compile pipeline is generated' do
        it_behaves_like 'pipeline checker', 'simple-depls-bosh-precompile-generated.yml', 'simple-depls-bosh-precompile-ref.yml'
      end
    end
  end
end
