...
describe Coa::EnvBootstrapper::Runner do
  ...
  describe '#run' do
    ...
    let(:env_creator_adapter) { Coa::EnvBootstrapper::EnvCreatorAdapter.new("bucc", {}) }
    ...
    context "when passed a configuration deactiving steps" do
      let(:inactive_steps_yml_path) { File.join(fixtures_dir('lib'), 'coa', 'env_bootstrapper', 'inactive_steps.yml') }
  ...
  describe '#run_pipeline_jobs' do
    context 'when the concourse creds are provided' do
      ...
      let(:bosh_prereqs_path) { File.join(fixtures_dir('lib'), 'coa', 'env_bootstrapper', 'bosh-prereqs.yml') }
      let(:concourse_prereqs_path) { File.join(fixtures_dir('lib'), 'coa', 'env_bootstrapper', 'concourse-prereqs.yml') }
      let(:runner) do
      ...
      let(:bosh) { Coa::EnvBootstrapper:: Bosh.new(bosh_config["bosh"]) }
      let(:concourse) { Coa::EnvBootstrapper:: Concourse.new(concourse_config["concourse"]) }
      let(:git_server_ip) { "1.1.1.1" }
