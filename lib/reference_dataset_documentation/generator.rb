module ReferenceDatasetDocumentation
  class Generator
    ...
    def config_repo_path
      @config_repo_path ||= File.join("#{reference_dataset_path}/#{config_repo_name}")
    end

    def template_repo_path
      @template_repo_path ||= File.join("#{reference_dataset_path}/#{template_repo_name}")
    end
    ...
    def check_params(params)
      errors = []
      required_params = %w[root_deployment_name config_repo_name template_repo_name path]
