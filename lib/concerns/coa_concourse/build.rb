...
module CoaConcourse
  ...
  class Build
    ...
    def self.watch_job(full_job_name, max_retries, fly_client)
      # logger.log_and_puts :info, "Watching job #{full_job_name}"
      retries = 0
      while retries < max_retries
        build = get_last_job_build(full_job_name, fly_client)
        sleep 1 unless ENV['test'] # yes, sleep even if the build is finished!
    ...
    def self.get_last_job_build(full_job_name, fly_client)
      raw_builds = fly_client.get_raw_job_builds(full_job_name)
