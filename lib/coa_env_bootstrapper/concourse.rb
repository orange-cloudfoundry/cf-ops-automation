...
module CoaEnvBootstrapper
  class Concourse
    ...
    def unpause_pipelines
      login_into_fly
      run_cmd "fly --target #{creds["target"]} unpause-pipeline --pipeline bootstrap-all-init-pipelines"
    end

    def trigger_jobs
      login_into_fly
      run_cmd "fly --target #{creds["target"]} trigger-job --job bootstrap-all-init-pipelines/bootstrap-init-pipelines"
    end
...
fly --target #{creds["target"]} sync"
    ...
    def upload_pipelines_command(pipeline_credentials_yml)
      "fly --target #{creds["target"]} set-pipeline --non-interactive \
...
--var paas-templates-uri='git://#{ceb.git.server_ip}/paas-templates' \
--var secrets-uri='git://#{ceb.git.server_ip}/secrets' \
