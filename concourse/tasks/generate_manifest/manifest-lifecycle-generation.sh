...
if [ $? -eq 3 ]; then
    echo "ignoring Iaas customization. IAAS_TYPE not defined / detected at ${YML_TEMPLATE_DIR}/<IAAS_TYPE>. IAAS_TYPE: <${IAAS_TYPE}>"
fi
...
for profile in ${NEWLINE_DELIMITED_PROFILES}; do
    ...
    if [ $? -eq 3 ]; then
        echo "ignoring ${profile} customization. Profile not defined / detected at ${YML_TEMPLATE_DIR}/<PROFILE>. Profile: <${profile}>"
    fi
done
set -e
